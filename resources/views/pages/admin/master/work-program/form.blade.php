...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form - Program Kerja </h6>
    </div>
    <div class="card-body row">
        <div class="col-xl-8 col-lg-7">
            <form method="POST" class="user" accept-charset="utf-8" enctype="multipart/form-data" action="<?= ($action == 'create') ? url('master-work-program') : url('master-work-program/'.$data['id']) ?>">
                @if ($action == 'edit')
                @method('patch')
                @endif
                @csrf
                ...
                <a href="<?php echo url('/master-work-program') ?>" class="btn btn-secondary">Kembali</a>
            </form>
        </div>
    </div>
</div>
